...
import Ormolu.Fixity (FixityMap, buildFixityMap, validateFixityConfig)
...
ormolu cfgWithIndices path str = do
  ...
  fixityConfig' <- fromEitherWith OrmoluInvalidFixityConfig $ validateFixityConfig (cfgFixityConfig cfg)
  let fixityMap = buildFixityMap fixityConfig'
  (warnings, result0) <-
