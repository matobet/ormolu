...
-- TODO
p_fieldLabels :: [XRec GhcPs (DotFieldOcc GhcPs)] -> R ()
p_fieldLabels = sep (txt ".") p_ldotFieldOcc
...
p_hsExpr' s = \case
  ...
  HsRecSel _ FieldOcc {..} -> p_rdrName foLabel -- TODO p_fieldOcc?
  HsOverLabel _ v -> do
    txt "#"
    atom v
  ...
  RecordCon {..} -> do
    ...
    let HsRecFields {..} = rcon_flds
        p_lhs = p_rdrName . foLabel . unLoc -- TODO unLoc OK?
  ...
  RecordUpd {..} -> do
    ...
    inci . braces N $
      either
        ...
        (p_recFields $ located' $ coerce p_fieldLabels)
        rupd_flds
  HsGetField {..} -> do
...
-- TODO introduce p_fieldOcc?

-- TODO unify with other hsFieldBind
p_pat_hsFieldBind :: HsRecField GhcPs (LPat GhcPs) -> R ()
p_pat_hsFieldBind HsFieldBind {..} = do
  located hfbLHS $ p_rdrName . foLabel
  unless hfbPun $ do
    space
    equals
    breakpoint
