{
  description = "Ormolu: a formatter for Haskell source code";
  inputs = {
    haskell-nix = {
      type = "github";
  ...
  outputs = { self, haskell-nix, nixpkgs, flake-utils }:
    flake-utils.lib.eachSystem [ "x86_64-linux" "x86_64-darwin" ] (system:
      let
            ...
            [
              ({ lib, ... }: {
                ...
                options.nonReinstallablePkgs =
                  ...
                  let adapt = ps:
                    if lib.hasPrefix "ghc9" ormoluCompiler
        ...
        # ormoluLive = hsPkgs.projectCross.ghcjs.hsPkgs.ormolu-live.components.exes.ormolu-live.overrideAttrs (_: pkgs.lib.optionalAttrs (!ormoluLiveLink) {
        #   installPhase = ''
        #     mkdir -p $out
        #   '';
        # });
      ...
      in
      {
        defaultPackage = ormoluExe;
        packages = {
          inherit ormoluLib ormoluExe ormoluCompiler;
        };
        apps = {
          ormolu = flake-utils.lib.mkApp { drv = ormoluExe; };
        };
