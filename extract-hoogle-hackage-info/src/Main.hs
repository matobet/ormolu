...
format f p = toStrict $ Format.format f p

-- | Put a formatted string
putFmtLn :: forall ps. Params ps => Format -> ps -> IO ()
putFmtLn f p = putStrLn $ format f p
...
getPackageName hoogleDatabasePath filePath = do
  when (not (hoogleDatabasePath `isPrefixOf` filePath)) $
    throwIO . SimpleException $
      format
        "{} do not start with {}"
        (pack filePath, pack hoogleDatabasePath)
  let packageName =
        stripSuffix' "/" $
          pack . head . splitPath $
            makeRelative hoogleDatabasePath filePath
      stripSuffix' suffix string = fromMaybe string $ stripSuffix suffix string
  when (Text.null packageName) $
    throwIO . SimpleException $
      format
...
readFileUtf8Latin1 filePath = catch @IOException (readFile filePath) $ \e -> do
  putFmtLn
...
onSymbolDecl packageName state@State {..} declOpName =
  let sPackageToOps' = case HashMap.lookup packageName' sPackageToOps of
      ...
      normOpName = declToNormName . unpack $ declOpName
      packageName' = unpack packageName
...
onFixityDecl packageName state@State {..} (rawFixDir, rawFixPrec, infixOpName) =
  let (sPackageToOps', sConflicts') = case HashMap.lookup packageName' sPackageToOps of
      ...
      packageName' = unpack packageName
      normOpName = infixToNormName $ unpack infixOpName
      fixDecl =
        let fixPrec = readT $ rawFixPrec
         in FixityInfo
              { fixDir = Just (readFixDir . unpack $ rawFixDir),
                fixMinPrec = fixPrec,
                fixMaxPrec = fixPrec
              }
      readFixDir = \case
  ...
  let soup = filterBlankTags $ parseTags content
      ...
      processRow tags = case extractText <$> groupOn "td" tags of
        rawName : rawDlCount : _ -> return $ Just (name, dlCount)
          where
            name = unpack . strip . head $ split (== ' ') rawName
            dlCount = readT $ strip rawDlCount :: Int
        _ -> do
          putFmtLn
            "Invalid line: {}"
            (Only $ intercalate " " $ showT <$> tags)
          return Nothing
      extractText tags = intercalate "" $ extractText' <$> tags
      extractText' = \case
        TagText t -> t
        _ -> ""
      groupOn _ [] = []
